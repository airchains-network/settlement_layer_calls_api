...
func HandlePostAddBatch(c *gin.Context, client cosmosclient.Client, ctx context.Context, account cosmosaccount.Account, addr string, dbIPaddress *leveldb.DB, sAPI string) {
	...
	// batchNumber, chainId, and inputs length can not be 0
	if batchNumber == 0 || len(chainId) == 0 || len(inputs) == 0 {
		respondWithError(c, "BatchNumber, ChainId, and Inputs cannot be empty")
		return
	}

	success, data, error_msg:= chain.AddBatch(batchNumber, chainId, inputs, client, ctx, account, addr, sAPI)
	if !success {
